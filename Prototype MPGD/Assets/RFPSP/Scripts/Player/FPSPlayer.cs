...
public class FPSPlayer : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (allowBulletTime)
		{//make bullet time an optional feature
			...
			if (Time.timeScale > 0 && !restarting)
			{//decrease or increase Time.timescale when bulletTimeActive is true
				...
				else
				{
					...
					if (1.0f - Mathf.Abs(Time.timeScale) > 0.05f)
					{//make sure that timescale returns to exactly 1.0f 
		...
		&& !FPSWalkerComponent.hideWeapon)
		{
			if (!zoomStartState)
			{
				...
				{//if button is tapped, toggle zoom state
					if (!zoomed)
					{
						zoomed = true;
					}
		...
		else
		{
			...
			if (!zoomEndState)
			{
				...
				if (zoomEnd - zoomStart > zoomDelay * Time.timeScale)
				{//if releasing zoom button after holding it down, stop zooming
					zoomed = false;
				}
			}
		}
		...
		&& ((FPSWalkerComponent.sprintStopTime + 0.4f) < Time.time))
		{
			...
			)
			{
				...
				{//if the object hit by the raycast is a pickup item and has the "Usable" tag
					...
					if (pickUpBtnState)
					{//check pickUpBtnState to prevent reticle from briefly showing custom/general pickup icon briefly when picking up last weapon before maxWeapons are obtained
						...
						if (hit.collider.gameObject.GetComponent<WeaponPickup>())
						{
							...
							&& PickupWeaponBehaviorComponent.addsToTotalWeaps)
							{//weapon adds to total inventory
								...
								&& !PickupWeaponBehaviorComponent.dropWillDupe)
								{
									...
									if (!useSwapReticle)
									{//if useSwapReticle is true, display swap reticle when item under reticle will replace current weapon
								...
								else
								{
									...
									{//weapon is removed on use, so show standard or custom pickup reticle
							...
							{//total weapons not at maximum and weapon under reticle does not add to inventory
				...
				else
				{
					...
					{//switch to pickup reticle if this NPC can be interacted with
						if (hit.collider.gameObject.GetComponent<AI>()
	...
	public void HealPlayer(float healAmt, bool isHungryThirsty = false)
	{
		...
		if (hitPoints < 1.0f)
		{//Don't add health if player is dead
			return;
		}
	...
	public void UpdateHunger(float hungerAmt)
	{
		...
		if (hitPoints < 1.0f)
		{//Don't add hunger if player is dead
			return;
		}
	...
	public void UpdateThirst(float thirstAmt)
	{
		...
		if (hitPoints < 1.0f)
		{//Don't add thirst if player is dead
			return;
		}
